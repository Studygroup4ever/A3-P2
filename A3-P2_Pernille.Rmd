...
for (fold in folds) {
  ...
  model = glmer(syntax, train, REML = F) 
  test <- test %>%
  mutate(pred = logit2prob(predict(model, newdata = test)),
         prediction = as.factor(ifelse(pred > .5, "Schizophrenia", "Control")))
  
  train <- train %>%
  mutate(pred = logit2prob(predict(model)),
         prediction = as.factor(ifelse(pred > .5, "Schizophrenia", "Control")))
  ...
  train_rocCurve = roc(response = train$Diagnosis, predictor = train$pred)
  test_rocCurve = roc(response = test$Diagnosis, predictor = test$pred)
